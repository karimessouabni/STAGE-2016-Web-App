...
if ($admin->estConnecte() == true) // si l'administrateur est deja loggé
{
    ...
    $result = $projet->insertInto(post('identifiant'), post('intitule'), post('reference'), post('superficie'), post('objectif'), post('consistance'), post('cout'), post('maitre'), post('naturem'), post('taux'), post('commune'), post('situation'), post('intervention'), post('naturet'), post('statut'), post('conventionY'), post('remarques'), post('long'), post('lat'));
